...
namespace Jaaq.Player.Api.Repositories
{
    public class InMemoryVideoRepository : IVideoRepository
    {
        ...
        private int _lastReturnedVideoIndex = 0;

        public Task<Video> GetRandomVideoAsync()
        {
            if (_lastReturnedVideoIndex == _videos.Count)
            {
                _lastReturnedVideoIndex = 0;
            }

            var video = _videos.Skip(_lastReturnedVideoIndex).First();
            _lastReturnedVideoIndex++;
